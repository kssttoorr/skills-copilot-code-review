document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchAnnouncements() {
    try {
      ...
      // Show first announcement prominently
      renderAnnouncementBanner(anns[0]);
    } catch (err) {
  ...
  function renderAnnouncementBanner(ann) {
    // ann.message may contain newlines; keep it short and readable
    announcementArea.innerHTML = `
  ...
  // Utility: format local datetime for input
  ...
  // Create/manage announcements modal
  function ensureManageModal() {
    ...
    modal.innerHTML = `
      <div class="modal-content">
        ...
        <form id="announcement-form" class="announcement-form">
          ...
          <textarea id="ann-message" placeholder="Message" required></textarea>
          <label>Start (optional)</label>
          <input type="datetime-local" id="ann-start" />
          <label>Expires (required)</label>
          <input type="datetime-local" id="ann-expires" required />
          <div class="manage-actions">
            <button type="button" id="ann-cancel" class="btn-secondary">Cancel</button>
            <button type="submit" id="ann-save">Save</button>
          </div>
        </form>
      </div>
    `;
    ...
    // Event listeners
    modal.querySelector(".close-announcements-modal").addEventListener("click", () => closeManageModal(modal));
    modal.addEventListener("click", (e) => { if (e.target === modal) closeManageModal(modal); });

    const form = modal.querySelector("#announcement-form");
    form.addEventListener("submit", async (e) => {
      e.preventDefault();
      await submitAnnouncementFromForm(modal);
    });

    modal.querySelector("#ann-cancel").addEventListener("click", () => {
      closeManageModal(modal);
    });

    return modal;
  }

  async function openManageAnnouncements() {
    if (!currentUser) {
      showMessage("You must be signed in to manage announcements.", "error");
      return;
    }
    const modal = ensureManageModal();
    await loadManageAnnouncements(modal);
  ...
  async function loadManageAnnouncements(modal) {
    const list = modal.querySelector("#announcements-list");
    list.innerHTML = "<p>Loading...</p>";
    try {
      const res = await fetch(`/announcements/all?username=${encodeURIComponent(currentUser.username)}`);
      ...
      if (anns.length === 0) {
        ...
        anns.forEach((a) => {
          ...
          item.innerHTML = `
            <div>
              ...
              <div class="announcement-meta">Expires: ${a.expires ? new Date(a.expires).toLocaleString() : '—'} ${a.start ? ' • Starts: ' + new Date(a.start).toLocaleString() : ''}</div>
              <div>${a.message}</div>
            </div>
            <div class="announcement-controls">
              <button class="edit-ann" data-id="${a.id}">Edit</button>
              <button class="delete-ann" data-id="${a.id}">Delete</button>
            </div>
          `;
        ...
        // Wire up edit/delete
        list.querySelectorAll('.edit-ann').forEach(btn => {
          btn.addEventListener('click', async (e) => {
            ...
            modal.querySelector('#ann-title').value = ann.title || '';
            modal.querySelector('#ann-message').value = ann.message || '';
            modal.querySelector('#ann-start').value = toLocalInputValue(ann.start);
            modal.querySelector('#ann-expires').value = toLocalInputValue(ann.expires);
            window.scrollTo({ top: modal.offsetTop, behavior: 'smooth' });
          });
        });

        list.querySelectorAll('.delete-ann').forEach(btn => {
  ...
  async function submitAnnouncementFromForm(modal) {
    const form = modal.querySelector('#announcement-form');
    const title = modal.querySelector('#ann-title').value.trim();
    const message = modal.querySelector('#ann-message').value.trim();
    const startVal = modal.querySelector('#ann-start').value;
    const expiresVal = modal.querySelector('#ann-expires').value;
    if (!title || !message || !expiresVal) {
      showMessage('Please fill title, message and expiration.', 'error');
      return;
    }
    ...
    payload.append('expires', new Date(expiresVal).toISOString());
    if (startVal) payload.append('start', new Date(startVal).toISOString());
    ...
    try {
      if (editing) {
        // Update
        const qs = `?username=${encodeURIComponent(currentUser.username)}` + `&title=${encodeURIComponent(title)}` + `&message=${encodeURIComponent(message)}` + `&expires=${encodeURIComponent(new Date(expiresVal).toISOString())}` + (startVal ? `&start=${encodeURIComponent(new Date(startVal).toISOString())}` : '');
        const res = await fetch(`/announcements/${editing}${qs}`, { method: 'PUT' });
        if (!res.ok) throw new Error('Failed to update');
        showMessage('Announcement updated', 'success');
      } else {
        // Create
        const res = await fetch(`/announcements?${payload.toString()}`, { method: 'POST' });
  ...
  async function deleteAnnouncement(id) {
    try {
      const res = await fetch(`/announcements/${id}?username=${encodeURIComponent(currentUser.username)}`, { method: 'DELETE' });
  ...
  // Wire manage button
  manageAnnouncementsButton.addEventListener('click', openManageAnnouncements);
  ...
  function updateAuthUI() {
    ...
    updateAuthBodyClass();
    // Refresh the activities and announcements to update the UI
    fetchActivities();
    fetchAnnouncements();

    // Show manage button only for signed in users
    if (currentUser) {
